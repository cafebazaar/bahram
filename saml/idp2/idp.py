...
from bahram import BahramUserManager
...
class SSO(Service):
    ...
    def verify_request(self, query, binding):
        ...
        try:
            self.binding_out, self.destination = IDP.pick_binding("assertion_consumer_service",
    ...
    def do(self, query, binding_in, relay_state="", encrypt_cert=None,
        ...
        if not _resp:
            ...
            try:
                ...
                _resp = IDP.create_authn_response(identity, userid=self.user, encrypt_cert_assertion=encrypt_cert,
                                                  **resp_args)
            ...
            if authz_info.startswith("Basic "):
                try:
                    ...
                    try:
                        ...
                        if user_manager.verify_email_and_password(user, passwd):
                            resp = Unauthorized()
                        self.user = user
                        self.environ[
...
def verify_username_and_password(dic):
    ok = user_manager.verify_email_and_password(dic["login"][0], dic["password"][0])
    return ok, (dic["login"][0] if ok else "")
...
class NMI(Service):
    def do(self, query, binding, relay_state="", encrypt_cert=None):
        ...
        name_id = IDP.ident.handle_manage_name_id_request(request.name_id, request.new_id, request.new_encrypted_id,
                                                          request.terminate)
...
if __name__ == '__main__':
    ...
    user_manager = BahramUserManager(CONFIG.ETCD_HOST, CONFIG.ETCD_PORT)
